    ...
    for (const file of files)
      if (!file.type.startsWith("image/") || file.size > MAX_FILE_SIZE)
        return NextResponse.json(
          { error: "Each file must be an image under 12MB." },
          { status: 400 },
        );
    const [item] = await db
      .insert(wardrobeItems)
      .values({ userId, name: "New garment" })
      .returning();
    const saved = await Promise.all(
      files.map(async (file) => storage.saveImage(Buffer.from(await file.arrayBuffer()), userId)),
    );
    await db.insert(itemPhotos).values(
      saved.map((image, position) => ({
        itemId: item.id,
