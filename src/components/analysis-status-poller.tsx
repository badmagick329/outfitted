...
function snapshot(items: StatusItem[]) {
  return items.map((item) => `${item.id}:${item.status}:${item.updatedAt}`).sort().join("|");
}
  ...
  useEffect(() => {
    async function poll() {
      ...
      try {
        ...
        if (previousSnapshot.current !== null && previousSnapshot.current !== nextSnapshot) router.refresh();
